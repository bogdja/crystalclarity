...
class ControllerCheckoutOctSmartcheckout extends Controller {
    ...
    public function shoppingCart($render = false, &$data = array()) {
        ...
        $shipping_method = isset($this->request->post['shipping_method']) && !empty($this->request->post['shipping_method']) ? $this->cleanAndReplace($this->request->post['shipping_method']) : '';
        ...
        if ($freeShippingFrom || $minimumOrderAmount) {
            ...
            $shipping_key = null;
            $original_shipping_value = 0;

            $codes = array_column($totals, 'code');
            $shipping_key = array_search('shipping', $codes);
            $sub_total_key = array_search('sub_total', $codes);

            $totalWithoutShipping = 0;

            if ($sub_total_key !== false) {
                $totalWithoutShipping += $totals[$sub_total_key]['value'];
            }

            foreach ($totals as $key => $total) {

                if ($total['code'] == 'sub_total') {
                    if (isset($oct_smart_checkout_data['minimum_order_amount']) && !empty($oct_smart_checkout_data['minimum_order_amount']) && $oct_smart_checkout_data['minimum_order_amount'] > $total['value']) {
                        $data['error_warning'] = sprintf($this->language->get('error_minimum_sum'), $this->currency->format($oct_smart_checkout_data['minimum_order_amount'], $this->session->data['currency']));
                    } 
                }
            ...
            if ($totalWithoutShipping >= $freeShippingFrom && $shipping_key !== false) {
                
                $original_shipping_value = $totals[$shipping_key]['value'];
                $totals[$shipping_key]['value'] = 0;
                $data['totals'][$shipping_key]['value'] = 0;
                $data['totals'][$shipping_key]['text'] = $this->currency->format(0, $this->session->data['currency']);
    ...
    private function confirmCheckout($render = false, $data = array()) {
        ...
        $sub_total_key = array_search('sub_total', array_column($totals, 'code'));
        $shipping_key = array_search('shipping', array_column($totals, 'code'));
        $reward_key = array_search('reward', array_column($totals, 'code'));
        $total_key = array_search('total', array_column($totals, 'code'));

        if ($freeShippingFrom && $sub_total_key !== false && isset($totals[$sub_total_key]) && $totals[$sub_total_key]['value'] >= $freeShippingFrom) {
            if ($shipping_key !== false && $total_key !== false) {
                $original_shipping_value = $totals[$shipping_key]['value'];
                $totals[$shipping_key]['value'] = 0;
                $totals[$total_key]['value'] -= $original_shipping_value;
        ...
        if ($reward_key !== false && isset($totals[$reward_key])) {
            $total_data['total'] = $totals[$total_key]['value'];
        }
