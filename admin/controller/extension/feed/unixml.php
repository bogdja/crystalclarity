...
class ControllerExtensionFeedUnixml extends Controller {
  ...
  //index - старт админки
    public function index() {
      ...
      foreach($data['system_limits'] as $limit_item => $limit_unit){
        $data[$limit_item] = ini_get($limit_item) . ($limit_unit?$limit_unit:'');
      }

      $max_allowed_packet = $this->db->query("SHOW VARIABLES LIKE 'max_allowed_packet'");
      $data['max_allowed_packet'] = round(($max_allowed_packet->row['Value']/1024/1024), 3) . 'M';

      $data['languages'] = array();
      $lang_get = $this->unixml->getLanguage();

      foreach(array('ua','en','ru') as $lang){
        $data['languages'][$lang] = array(
          'link' => $this->url->link($this->unixml->path . '/language', $this->unixml->token() . '=' . $this->session->data[$this->unixml->token()] . '&type=feed&lang=' . $lang, true),
          'active' => ($lang_get == $lang)?true:false
        );
      }
  ...
  //language - смена языка в модуле
    public function language(){
      ...
      if(isset($this->request->get['lang'])){
        $lang_get = $this->request->get['lang'];

        if($lang_query->num_rows){
          $this->db->query("UPDATE " . DB_PREFIX . "unixml_setting SET value = '" . $this->db->escape($lang_get) . "' WHERE code = 'language'");
        }else{
          $this->db->query("INSERT INTO " . DB_PREFIX . "unixml_setting SET value = '" . $this->db->escape($lang_get) . "', code = 'language'");
        }
      }
  ...
  //Функции импорта
    ...
    //getImportProgress - функция получения прогресса по импорту
      public function getImportProgress(){
        $stat = array();
        if(is_file(DIR_SYSTEM . 'unixml/import/' . $this->request->get['import_id'] . '.uxs')){
          ...
          //проверка еще раз после 3 сек ожидания
            if(isset($this->request->post) && $this->request->post){
              foreach($this->request->post as $before_id => $before_fize){
                $stat_item = false; //процесс не идет
                if(isset($prices[$before_id])){
                  if($before_fize != $prices[$before_id]){
                    $stat_item = true; //если за 3 сек изменилось то процесс идет
                  }
                }
                $prices[$before_id] = $stat_item;
              }
            }
          //проверка еще раз после 3 сек ожидания
    ...
    //importClearData - функция очистки данных импорта
      public function importClearData() {
        ...
        foreach(array('xml','uxs','uxp','uxc') as $ext){
    ...
    //importSavePriceSetting - функция сохранения настроек импорта
      public function importSavePriceSetting($setting_data = false) {
        ...
        if(isset($this->request->post['unixml_import_xml_product_multilang']) && $this->request->post['unixml_import_xml_product_multilang']){
          foreach($this->request->post['unixml_import_xml_product_multilang'] as $key => $value){
            if($value['attribute']){
              $this->request->post['unixml_import_xml_product_multilang'][$key]['attribute'] = str_replace("'", "&apos;", $value['attribute']);
            }
          }
        }
  ...
  //Функции экспорта
    ...
    //exportLoadFeedSetting - функция загрузки параметров конкретного фида
      public function exportLoadFeedSetting() {
        ...
        if(!$data['unixml_store']){
          $data['unixml_store'] = array_column($data['stores'], 'store_id');
        }
        ...
        if($data[$this->unixml->varname('xml_name')]){
    ...
    //exportGetAttributeList - функция выборки атрибутов
      private function exportGetAttributeList($feed){
        ...
        foreach($query->rows as $row){
          $attribute_name_query = $this->db->query("SELECT name FROM " . DB_PREFIX . "attribute_description WHERE attribute_id = '" . (int)$row['attribute_id'] . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
  ...
  //Функции вспомогательные
          ...
          if(substr($row['query'], 0, 11) == 'product_id='){ //товары
            ...
            if($product_query->num_rows){
              ...
              $urls[$row['query']] = array(
                'type' => $this->unixml->text('text_product') . ' (id:' . $product_id . ')',
    ...
    //serviceUrlSave - функция сохранение ЧПУ
      public function serviceUrlSave(){
        $seo_index = $this->request->post['seo_index'];
        $seo_keyword = $this->request->post['seo_keyword'];

        if($seo_index && $seo_keyword){
          $data = array();
          $this->unixml->importGetAliasTable($data);

          $this->db->query("UPDATE " . DB_PREFIX . $data['table'] . " SET keyword = '" . $this->db->escape($seo_keyword) . "' WHERE seo_url_id = '" . (int)$seo_index . "'");
        }
