...
class ModelOCTemplatesMainOctSettings extends Model {
        ...
        if ($data) {
            ...
            foreach ($data as $menu) {
                ...
                if (isset($menu['setting']) && $menu['setting'] && empty($menu['settings'])) {
                    ...
                    $this->db->query("UPDATE `" . DB_PREFIX . "oct_menu` SET `type` = '" . $this->db->escape($menu['type'])  . "', `display_option` = '" . $this->db->escape($menu['display_option'])  . "', `settings` = '" . $this->db->escape(json_encode($menu['settings'], true))  . "', `sort_order` = '" . (int)$menu['sort_order']  . "', `status` = '" . (int)$menu['status']  . "' WHERE `oct_menu_id` = '" . (int)$menu['setting'] . "'");

                    $oct_menu_id = (int)$menu['setting'];
                } else {
                    $this->db->query("INSERT INTO `" . DB_PREFIX . "oct_menu` SET `type` = '" . $this->db->escape($menu['type'])  . "', `display_option` = '" . $this->db->escape($menu['display_option'])  . "',  `settings` = '" . $this->db->escape(json_encode($menu['settings'], true))  . "', `sort_order` = '" . (int)$menu['sort_order']  . "', `status` = '" . (int)$menu['status']  . "'");
    ...
    public function getMenuItems() {
        ...
        foreach ($query->rows as $menu_item) {
            ...
            $oct_menu_data[$menu_item['oct_menu_id']] = [
                ...
                'type' => $menu_item['type'],
				'display_option' => $menu_item['display_option'],
    ...
    public function getMenuItemsByMenuId($menu_id) {
        ...
        if ($query->num_rows) {
            $oct_menu_data = [
                ...
                'type' => $query->row['type'],
				'display_option' => $query->row['display_option'],
    ...
    public function installOCTMenu() {
        $sqls[] = "
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oct_menu` (
			  ...
			  `display_option` varchar(20) NOT NULL,
	...
	public function installReviewsTables() {
	...
	public function editOctReviewData($review_id, $data) {
		if (!$this->checkIfReviewExists($review_id)) {
			$this->addOctReviewData($review_id, $data);
			return;
		}	
		$this->db->query("UPDATE " . DB_PREFIX . "oct_product_reviews SET admin_answer = '" . $this->db->escape($data['admin_answer']) . "', positive_text = '" . $this->db->escape($data['positive_text']) . "', negative_text = '" . $this->db->escape($data['negative_text']) . "' WHERE review_id = '" . (int)$review_id . "'");
	}

	public function addOctReviewData($review_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "oct_product_reviews SET review_id = '" . (int)$review_id . "', positive_text = '" . $this->db->escape($data['positive_text']) . "', negative_text = '" . $this->db->escape($data['negative_text']) . "', admin_answer = '" . $this->db->escape($data['admin_answer']) . "'");
	}

	public function deleteOctReviewData($review_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "oct_product_reviews WHERE review_id = '" . (int)$review_id . "'");
	}

	public function checkIfReviewExists($review_id) {
		$query = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "oct_product_reviews WHERE review_id = '" . (int)$review_id . "'");
